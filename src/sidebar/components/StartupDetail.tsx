...
import { MapPin, Users, Building2, ExternalLink, Globe, Check, Calendar } from 'lucide-react'
          ...
          <SheetHeader className="p-4 pb-0">
            <div className="flex items-start gap-3">
              {startup.small_logo_thumb_url ? (
                <img src={startup.small_logo_thumb_url} alt="" className="w-12 h-12 rounded-xl object-contain bg-muted shrink-0 ring-1 ring-border"
                  onError={(e) => {
                    (e.target as HTMLImageElement).outerHTML =
                      `<div class="w-12 h-12 rounded-xl bg-yc-orange/10 flex items-center justify-center text-yc-orange font-bold text-lg shrink-0 ring-1 ring-yc-orange/20">${startup.name[0]}</div>`
                  }}
                />
              ) : (
                <div className="w-12 h-12 rounded-xl bg-yc-orange/10 flex items-center justify-center text-yc-orange font-bold text-lg shrink-0 ring-1 ring-yc-orange/20">
                  {startup.name[0]}
                </div>
              )}
          ...
          <ScrollArea className="flex-1 p-4 pt-3">
            <div className="space-y-4">
              <div className="flex flex-wrap gap-1.5">
                <Badge variant="orange" className="font-medium">{startup.batch}</Badge>
              ...
              <div className="grid grid-cols-2 gap-4">
                {startup.website && (
                  <div className="space-y-1">
                    <h4 className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1">
                      <Globe className="h-3 w-3 text-yc-orange/70" /> Website
                    </h4>
                    <a href={startup.website} target="_blank" rel="noreferrer" className="text-sm text-primary hover:underline truncate block">
                ...
                {(startup.location || startup.all_locations) && (
                  <div className="space-y-1">
                    <h4 className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1">
                      <MapPin className="h-3 w-3 text-yc-orange/70" /> Location
                    </h4>
                    <p className="text-sm truncate">{startup.all_locations || startup.location}</p>
                  </div>
                )}
                {startup.team_size && (
                  <div className="space-y-1">
                    <h4 className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1">
                      <Users className="h-3 w-3 text-yc-orange/70" /> Team
                    </h4>
                    <p className="text-sm">{startup.team_size} people</p>
                  </div>
                )}
                {startup.year_founded && (
                  <div className="space-y-1">
                    <h4 className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1">
                      <Building2 className="h-3 w-3 text-yc-orange/70" /> Founded
                    </h4>
              ...
              {startup.long_description && (
                <div>
                  <h4 className="text-xs font-medium text-muted-foreground mb-1.5">About</h4>
                  <p className="text-sm text-muted-foreground/90 leading-relaxed">{startup.long_description}</p>
                </div>
              )}
              ...
              <div>
                <h4 className="text-xs font-medium text-muted-foreground mb-2 flex items-center gap-1.5">
                  <span className="w-1 h-3 rounded-full bg-yc-orange" /> Lists
                </h4>
                <div className="space-y-1">
                  {collections.map((col) => {
                    ...
                    return (
                      <div
                        key={col.id}
                        className="flex items-center gap-2.5 px-2.5 py-2 text-xs rounded-md cursor-pointer hover:bg-accent select-none transition-colors"
                        onClick={() => handleToggleList(col.id)}
                      >
                        <div
                          className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors ${
                            inList ? 'bg-primary border-primary text-primary-foreground' : 'border-muted-foreground/50 hover:border-foreground'
                          }`}
                        >
                          {inList && <Check className="h-2.5 w-2.5" />}
                        </div>
                        <span className={`flex-1 ${inList ? 'font-medium text-foreground' : 'text-muted-foreground'}`}>{col.name}</span>
                        <span className="text-[10px] text-muted-foreground/60">{col.startupIds.length}</span>
                      </div>
                    )
                  })}
                </div>
              ...
              {startup.founders && startup.founders.length > 0 && (
                <>
                  ...
                  <div>
                    <h4 className="text-xs font-medium text-muted-foreground mb-2 flex items-center gap-1.5">
                      <span className="w-1 h-3 rounded-full bg-yc-orange" /> Founders
                    </h4>
                    <div className="space-y-2">
                      {startup.founders.map((f, i) => (
                        <div key={i} className="flex items-center gap-2.5 p-2 rounded-md hover:bg-accent transition-colors">
                          {f.avatar_url ? (
                            <img src={f.avatar_url} alt="" className="w-8 h-8 rounded-full object-cover bg-muted ring-1 ring-border" />
                          ) : (
                            <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-xs font-medium ring-1 ring-border">{f.name[0]}</div>
                          )}
                          ...
                          <div className="flex gap-0.5 shrink-0">
                            {f.linkedin_url && (
                              <a href={f.linkedin_url} target="_blank" rel="noreferrer">
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-[#0a66c2]">
                            ...
                            {f.twitter_url && (
                              <a href={f.twitter_url} target="_blank" rel="noreferrer">
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-sky-500">
              ...
              <a href={ycUrl} target="_blank" rel="noreferrer" className="block">
                <Button variant="outline" size="sm" className="w-full gap-2 text-xs h-9">
                  <ExternalLink className="h-4 w-4 text-yc-orange" /> View on Y Combinator
                </Button>
              </a>
            </div>
          </ScrollArea>
